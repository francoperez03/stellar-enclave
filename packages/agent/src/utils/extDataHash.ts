 ...
 *
 * Duplicated from facilitator/src/utils/extDataHash.ts so @enclave/agent can
 * independently compute the binding hash used as a circuit public input. Keeping
 * @enclave/core dep-light (no crypto / stellar-sdk deps) is deliberate — both agent
 * and facilitator own their own keccak port.
 *
 * CRITICAL: do NOT change the field ordering or XDR serialization. The sorted-ScMap
 * pattern is the ONLY way to match the on-chain contract.
 */
import { keccak_256 } from "@noble/hashes/sha3.js";
import { Address, XdrLargeInt, xdr } from "@stellar/stellar-sdk";
import type { ExtData } from "../types.js";
...
export interface HashExtDataResult {
  ...
  /** decimal string form of bigInt — matches circuit witness input format */
  decimal: string;
}
...
export function hashExtData(ext: ExtData): HashExtDataResult {
  ...
  return { bigInt: reduced, bytes, hex: bytesToHex(bytes), decimal: reduced.toString() };
}
