...
- [ ] **SDK-05**: Agent private keys are read from environment variables or local files outside the repo; `.gitignore` blocks `*.key`, `.env`, `secrets/`, `wallets/`
- [ ] **SDK-06**: The SDK emits structured logs with automatic redaction of keys, nullifiers, and raw payloads; a redaction test confirms no secret survives through the log pipeline
